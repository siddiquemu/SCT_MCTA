...
from .utils import bbox_overlaps, center_distances, ReID_search_distances, warp_pos, \
	get_center, get_height, get_width, make_pos
...
class Tracker:
	...
	def __init__(self, obj_detect, reid_network, tracker_cfg, global_track_num,
		...
		self.results = {}
		self.det_time = det_time_counter
	...
	def step(self, blob, detections, cam=None):
		...
		#self.det_time[cam].append(time.time() - det_start)
		##################
		# Predict tracks #
		##################
		...
		if det_pos.nelement()>0:
			...
			if len(self.inactive_tracks)>=1:
				...
				if new_det_pos.nelement() > 0 and cam in [330, 360, 361, 340, 440, 300] and self.isSCT_ReID:
					...
					#print('cam: {}, double reid time: {:.2f} sec'.format(cam, time.time() - start_reid))
		...
		for t in self.inactive_tracks:
			t.count_inactive += 1
			#if int(t.id) not in self.reid_rads_patience.keys():
				#self.reid_rads_patience[int(t.id)] = self.inactive_patience#t.count_inactive
				#self.reid_radious[int(t.id)] = [60,
											#self.inactive_patience]
			#else:
				#self.reid_rads_patience[int(t.id)] += 1
				#self.reid_radious[int(t.id)][0] += 10 #compute radius based on velocity or fov center
				#self.reid_radious[int(t.id)][1] += 1
				#TODO: reset the rads and patience when cross the limit
				#if self.reid_rads_patience[int(t.id)]>=200:
					#self.reid_rads_patience[int(t.id)] = 200
					#self.reid_radious[int(t.id)] =  [200, 200]  # compute radius based on velocity or fov center
			#print(self.reid_rads_patience.keys())
			#print('inactive tracks: {}, patience: {}, rads: {}'.format(t.id, self.reid_radious[int(t.id)][1],
																	   #self.reid_radious[int(t.id)][0]))
