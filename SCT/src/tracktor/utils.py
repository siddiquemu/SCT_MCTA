...
def filter_tracklets(num_frames, trackers, start_frame=None, batch_length=120, min_trklt_size=30, reid_patience=30):
    ...
    look_back_frames = np.arange(num_frames-2*batch_length+1, num_frames+1)
    for j, tl in trackers.items():
        ...
        if len(set(np.array(list(tl.keys()))+start_frame).intersection(set(list(batch_frames))))>0 and len(tl.keys())>=min_trklt_size//2:
    ...
    for j, t in tracks.items():
        ...
        if num_frames-1 in t.keys():
            ...
            ax.annotate(j, (t_i[0] + (t_i[2] - t_i[0]) / 2.0, t_i[1] + (t_i[3] - t_i[1]) / 2.0),
                        color='red', weight='bold', fontsize=20, ha='center', va='center')
