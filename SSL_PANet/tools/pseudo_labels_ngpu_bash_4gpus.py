...
class Pseudo_Labels(object):
    ...
    def get_masks(maskRCNN, detPB, im=None, vis_img=None, im_name=None, fr=None, pred_score=None, dataset=None,
        ...
        if det_size > 0:
            ...
            det_class, mask_class, _ = get_box_mask_single_cat(fr, cls_boxes, cls_segms, proposals=detPB, thresh=0,
                                                 ...
                                                 cls_segms_coarse=class_segms_coarse, img=im, rgr_refine=apply_rgr)
        else:
            mask_class = []
            det_class = []
                ...
                if cls_boxes is not None:
                    ...
                    if (len(dets) > 0):
                        for i, box in enumerate(dets):
                            ...
                            elif self.init_params['database'] in  ['clasp1', 'clasp2']:
                                if box[6] >= 0.5:
                                    self.detPB.append(box)
                                    self.maskPB_rot.append(masks_rot[i])
                                    self.detPB_rot.append(dets_rot[i])

                            # elif self.init_params['database'] in  ['clasp2']: #discard the ROI as the videos are not being annotated
                            #     if box[4] < 960 and (box[6] >= 0.5 and box[7] == 1): #person
                            ...
                            #             self.detPB_rot.append(dets_rot[i])
                            #
                            #     if box[4] < 960 and (box[6] >= 0.5 and box[7] != 1): #bag
                            #         if cam_path.split('/')[-1] in ['G_9', 'H_9', 'I_9'] and (
                            #                 (box[2] + box[4] / 2) < 1500
    ...
    def pseudo_labels_selection(self, fr_num, cluster_modes, det_indexs):
        ...
        if len(cluster_modes)>0 or len(anns_fr) > 0:
            if self.init_params['database']=='flower':
                ...
                self.pseudo_labels_angles = [6, 12, 90, 186, 270, 354] #[6, 84, 90, 96, 174, 180, 186, 270, 354]
    ...
    def pseudo_labels_main(self):
        ...
        """
        total_frames = len(self.img_subset)
        fr_count = 1
        for fr_num in random.sample(self.img_subset, len(self.img_subset)):
            ...
            if fr_num in self.semi_frames:
                print(f"PID: {self.p_id} >> frames remaining: {total_frames-fr_count}, Labeled Frame: {fr_num}, Execution time {time.time() - start_time} secs")
            else:
                print(f"PID: {self.p_id} >> frames remaining: {total_frames-fr_count}, Unlabeled Frame: {fr_num}, Execution time {time.time() - start_time} secs")
            fr_count+=1
...
def parse_args():
    ...
    parser.add_argument(
        '--num_gpus',
        help='Number of GPUs for pseudo-labels generation',
        default=2, type=int)
...
if __name__ == '__main__':
    ...
    model_type = 'modified_loss' #'modified_loss_semi' 'modified_loss_wo_reg'
    exp = args.ssl_iter
    ...
    if args.database in ['clasp1', 'clasp2']:
        ...
        if num_gpus==3:
           init_params['cuda_list'] = [0,0,1,1,2,2]

        init_params['apply_RGR'] = False
        init_params['det_thr'] = 0.5
        init_params['class_list'] = [1,2]
        init_params['nms_thr'] = 0.5
        init_params['cluster_score_thr'] = [0.1, 0.1]  #clasp2: [0.1, 0.1] clasp1: [0.4, 0.2]
