...
def main(maskRCNN, dataset, output_dir, image_dir, out_path, angleSet,
    ...
    for i in xrange(num_images):
        if database not in ['clasp-5k', 'DOTA', 'iSAID']:
...
if __name__ == '__main__':
    ...
    NAS = '/media/NAS/LabFiles/Walden/trainTestSplit/'
    base_dir = NAS + 'test/dataFormattedProperly/splitImages4x3/'

    gpu_id = 0
    storage = '/media/abubakarsiddique/'
    # required inputs
    database = 'clasp2'  # 'clasp1_30fps' #'clasp1' #'flower' #'clasp1' #'PVD' #'clasp1_30fps' #'clasp1' #'clasp2_30fps' #'clasp2' #'clasp1_30fps'#'MOT20'#'iSAID'#'clasp1'#'DAVIS'#'COCO' #'clasp1' #'AICity' #'PVD'#'clasp1' #'DOTA'#'clasp1'#'clasp1'#'clasp-5k'#'wild-track'#'clasp-5k'#'wild-track'#'logan'#'kri', 'wild-track'

    # model and loss
    iter = 1
    baseline = 'ResNet50_box_mask_tuned'  # 'ResNet50_box_mask_tuned' #'ResNet50'#'ResNet50_box_mask_tuned' #'ResNet50' #'ResNet50_box_mask_tuned' #'ResNet50'#'ResNet50DOTA'#'ResNet50_box_mask_tuned0' #'ResNet50AICity' #'ResNet50_box_mask_tuned0' # 'ResNet50DOTA'#'ResNet50_box_mask_tuned0'#'ResNet50_box_mask_tuned0'#'ResNet50_box_mask_tuned0'#'ResNet50'#'ResNet50_tuned'  # 'ResNeXt101_tuned'
    SSL_alpha_loss = 1
    ...
    mask_model = f'{storage}PANet/Outputs/e2e_panet_R-50-FPN_2x_mask'
    mask_model = f'{storage}PANet_Models'
    det_model = f'{storage}PANet/Outputs/e2e_panet_R-50-FPN_1x_det'
    ...
    if baseline == 'ResNet50':
        cfg_file = f'{storage}PANet/configs/panet/e2e_panet_R-50-FPN_2x_mask.yaml'
        load_ckpt = f'{storage}PANet/panet_mask_step179999.pth'  # baseline
        data = 'coco'
        class_list = list(np.arange(1, 81))  # [1]#, 25, 27, 29] #list(np.arange(1,81)) #[1, 25, 27, 29]
        load_detectron = None

    if baseline == 'ResNet50AICity':
        cfg_file = f'{storage}PANet/configs/panet/e2e_panet_R-50-FPN_2x_mask.yaml'
        load_ckpt = f'{storage}PANet/panet_mask_step179999.pth'  # baseline
        data = 'coco'
        class_list = [3, 6, 8]
    ...
    if baseline == 'ResNet50_box_mask_tuned':
        ...
        if database in ['clasp1', 'clasp1_30fps']:
            ...
            if SSL_alpha_loss:
                ...
                #load_ckpt = mask_model + '/clasp1/modified_loss_mixed_aug/iter{}/ckpt/model_step9999.pth'.format(iter)
                print('alpha loss model: {}'.format(load_ckpt))
            elif SSL_alpha_loss_sigm:
    ...
    if database in ['clasp2_30fps']:
        cams = ['G_2', 'G_5', 'H_2', 'H_5', 'I_2',
                'I_5']  # ['G_9', 'G_11', 'H_9', 'H_11', 'I_9', 'I_11'] #['G_2', 'G_5', 'H_2', 'H_5', 'I_2', 'I_5']
    ...
    if database == 'clasp1_30fps':
        server = '/media/NAS/LabFiles/CLASP/CLASP_Data/Data_GT/' \
                 'ourdataset/exp6a/'
    ...
    for cam in cams:
        ...
        if database == 'clasp1':
            ...
            result_path = f'{storage}PANet_Results/CLASP1/PANet_mask/rotations_{len(angleSet)}'  # det result path for rotation experiment
            img_fmt = 'png'
        ...
        if database in ['clasp2', 'clasp2_30fps']:
            ...
            if test_aug:
                ...
                output_dir = storage + 'PANet_Results/CLASP2/PANet_mask/' + cam + '/resnet50-infer'
            #result_path = storage + 'PANet_Results/CLASP2/PANet_mask'  # det result path
            result_path = f'{storage}PANet_Results/CLASP2/PANet_mask/rotations_{len(angleSet)}'  # det result path for rotation experiment

            if database == 'clasp2_30fps':
                if SSL_alpha_loss:
                    output_dir = storage + 'PANet_Results/CLASP2/PANet_mask/SSL_alpha_dets_30fps/' + cam
                elif SSL_loss:
        ...
        main(maskRCNN, dataset, output_dir, image_dir, result_path, angleSet,
             saveAugResult=save_aug, vis=True, cam=cam, img_fmt=img_fmt, class_list=class_list,
             ...
             vis_rate=vis_rate, detector_infer_time=detector_infer_time, detector_test_aug_time=detector_test_aug_time)
