...
def main(folders, angleSet, dataset_clasp, output_dir, isTrain, imgResultDir, soft_nms=False,
...
if __name__ == '__main__':
    ...
    isTrain = False
    # required inputs
    data = 'clasp1_2021' #'coco'#  # 'clasp1_2021' #'clasp1_2021' #'clasp1_2021' #'coco' #'clasp2020' #
    ...
    cluster_score_thr = [0.6, 0.4] #0.5,0.4
    det_thr = 0.5 #0.5 #6 - 0.6

    data_type = 'clasp1_augMS'  # 'gt' #'augMS
    ...
    if data_type=='clasp1_augMS':
        folders = glob.glob(benchmark_path + '*_{}'.format(cam))
    folders.sort(key=lambda f: int(''.join(filter(str.isdigit, str(f)))))

    dataset_clasp, all_scores = main(folders, angleSet, dataset_clasp, SaveImgDir, isTrain, imgResultDir, soft_nms,
                         class_list, data_type, save_data, saveAugResult, cluster_score_thr, det_thr, all_scores)
    if save_data:
