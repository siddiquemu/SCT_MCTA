...
def regress_dets(maskRCNN, proposals=None, im=None, im_name=None, fr=None, pred_score=None, isNMS=0, nms_thr=0.3, dataset=None,
...
def main(all_frames, semi_frames, percent_clasp_gt, angleSet, dataset_clasp, input_imgs_dir,  output_dir, isTrain, imgResultDir, soft_nms=False,
         class_list=None, data_type='gt', save_data=False, saveAugResult=False, apply_cluster_mode=False,
        det_thr=0.5, nms_thr=0.3, all_scores=None, regress_cluster=None, vis_annos=False,
         cluster_score_cams=None, database='clasp1', gpu_id=1, verbose=False):
    """main function"""
    # configure detector
    ...
    for fr_num in random.sample(all_frames, len(all_frames)):
        ...
        detPB_rot = []
        masks_0 = None
        for angle in random.sample(angleSet, len(angleSet)):
            if angle>0:
                if verbose:
        ...
        if len(detPB) > 0 and saveAugResult:
                ...
                for cl in class_list:
                    ...
                    if len(det_cl[cl]) > 0:
                        class_dets = regress_dets(maskRCNN, proposals=det_cl[cl], im=im, im_name=os.path.basename(im_name), fr=fr_num,
                                                  pred_score=0.1, isNMS=0, nms_thr=0.4, dataset=dataset,
            ...
            if len(detPB)>0:
                ...
                if fr_num % 10 == 0:
                    ...
                    show_result = False
                cluster_score_thr = [0.2, 0.2] #cluster_score_cams[im_name.split('/')[-3]]
                MI_MS = Cluster_Mode(detPB, fr_num, angleSet, im,
                                ...
                                bw_type='estimated', im_name=im_name, verbose=verbose)
        ...
        if len(det_indexs) > 0 and len(detPB) > 0 and len(cluster_modes)>0 and save_data:
            ...
            pseudo_labels_angles = [6,  84, 90, 96,  174, 180, 186, 270, 354]
            pseudo_labels_angles = random.sample(pseudo_labels_angles, len(pseudo_labels_angles))
            # first angle should be zero
            pseudo_labels_angles.insert(0, 0)
            for theta in pseudo_labels_angles:#angleSet:
...
if __name__ == '__main__':
    ...
    det_thr = 0.5 #clasp2:0.5-0.7, clasp1:0.5
    #model selection
    if data == 'coco':
    ...
    dataset_clasp, all_scores = main(all_frames, semi_frames, percent_clasp_gt, angleSet, dataset_clasp, input_imgs_dir, SaveImgDir, isTrain, imgResultDir,
                                     ...
                                     database, gpu_id=gpu_id, verbose=verbose)
    if save_data:
