...
class Pseudo_Labels(object):
    ...
    def get_masks(maskRCNN, detPB, im=None, vis_img=None, im_name=None, fr=None, pred_score=None, dataset=None,
        ...
        if det_size > 0:
            ...
            det_class, mask_class = get_box_mask_single_cat(fr, cls_boxes, cls_segms, proposals=detPB, thresh=0,
                                                            ...
                                                            cls_segms_coarse=clas_segms_coarse, img=im,
                                                            rgr_refine=apply_rgr)
        else:
            mask_class = []
            det_class = []
        ...
        assert len(det_class) == len(mask_class) == len(
            detPB), f'{len(det_class)}=={len(mask_class)}=={len(detPB)}, {cls_boxes[class_id]}, {cls_segms[class_id]}'
    ...
    def vis_gt(im, boxs, gt_vis_path=None, imname=None):
        for bb in boxs:
            im = cv2.rectangle(im, (np.int(bb[2]), np.int(bb[3])), (np.int(bb[2] + bb[4]), np.int(bb[3] + bb[5])),
                               (0, 255, 0), 4)
    ...
    def regress_dets(self, proposals=None, fr=None, pred_score=None, isNMS=0, class_id=None):
        ...
        # assert len(class_list)==1, 'regression for augmented dets only workd for single class model.. mulit-class: full input proposals are used for all classes'
        assert det_size == dets_proposals.shape[0], 'proposals size {}, prediction size {}'.format(det_size,
                                                                                                   dets_proposals.shape[
                                                                                                       0])
    ...
    def get_annos_cluster_mode(self, cluster_modes, masks_0=None, fr_num=None,
        ...
        if angle == 0:
            ...
            if len(box_list) > 0:
                ...
                if self.vis_annos:
                    fig.savefig(os.path.join(self.vis_dir, '{}_{}'.format(angle, os.path.basename(self.im_name))),
                                dpi=200)
                    plt.close('all')
            # try:
            ...
            for cl in self.class_list:
                ...
                if len(det_cl[cl]) > 0:
                    det_cl[cl], mask_cl[cl], imgrot_vis = self.get_masks(self.maskRCNN, det_cl[cl], im=imgrot,
                                                                         vis_img=imgrot_vis,
                                                                         im_name=os.path.basename(self.im_name),
                                                                         fr=fr_num, pred_score=pred_score,
                                                                         dataset=self.dataset,
                                                                         class_list=self.class_list,
                                                                         vis_dir=self.vis_dir,
                                                                         vis=self.vis_annos, img_fmt='jpg',
        ...
        if len(cluster_modes) > 0 or len(anns_fr) > 0:
            ...
            for theta in self.pseudo_labels_angles:
                ...
                if self.init_params['database'] in ['clasp1', 'clasp2']:
                    ...
                    if self.verbose:
                        print(
                            f'instance certainty score: {fr_det[:, 6]}, rotation invariant score: {cluster_modes[:, 6]}')
                ...
                if len(fr_det) > 0:
                    ...
                    # save image info
                    imgrot = self.rot_imgs_dict[theta]
                    #apply color jitter and motion blur augmentation on the pseudo labels
                    if self.apply_mixed_aug:
                        imgs = self.get_mixed_aug_images(imgrot)
                    else:
                        imgs = [imgrot]
                    for i, im_aug in enumerate(imgs):
                        theta_new = int(f'{theta}{i}')
                        imgIdnew = 10000 * int('%06d' % fr_num) + theta_new
                        ...
                        if fr_num in self.semi_frames and i==0:
                        ...
                        for ib, box in enumerate(fr_det):
                            ...
                            if (box[6] >= self.cluster_score_thr[0] and box[7] == 1) or \
                                ...
                                annID = int(
                                    f'{ib + 1}{fr_num}{self.p_id + theta}')  # fr_num is unique in multiple precesses
...
if __name__ == '__main__':
    ...
    Challenge is the motion blur augmentation is not available in the torch <1.7  
    """
