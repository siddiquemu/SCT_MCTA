...
def get_all_dirs(args, exp, init_params, storage, models_dir, model_type, percent_gt=None,):
    ...
    if init_params['database'] in ['clasp1', 'clasp2']:
        if init_params['database'] == 'clasp2':
            init_params['train_img_dir'] = f'{args.data_dir}/train_gt_det/img1/'
        else:
            init_params['train_img_dir'] = f'{args.data_dir}/train_gt_det/img1/'

        if model_type=='modified_loss_semi':
            init_params['output_dir'] = f'{args.data_dir}/test_augMS_gt_score/iter{exp}'
            shutil.rmtree(init_params['output_dir'])
            ...
            init_params['model_path'] = os.path.join(models_dir, init_params['database'],
            ...
            init_params['output_dir'] = f'{args.data_dir}/test_augMS_gt_wo_regress/iter{exp}'
            if not os.path.exists(init_params['output_dir']):
                os.makedirs(init_params['output_dir'])
            ...
            init_params['output_dir'] = f'{args.data_dir}/test_augMS_gt_score_exp/iter{exp}'
            if not os.path.exists(init_params['output_dir']):
                os.makedirs(init_params['output_dir'])
            ...
            if exp>0:
                ...
                init_params['model_path'] = os.path.join(models_dir, init_params['database'], model_type,
                                                        f'iter{exp - 1}', 'ckpt/model_step19999.pth')
            else:#pretrained base model
                init_params['model_path'] = f'{storage}/models/R50-MRCNN/panet_mask_step179999.pth'
        ...
        if init_params['semi_supervised']:
            ...
            json_path = '/media/6TB_local/SoftTeacher/data/{}/annotations/semi_supervised/instances_train_{}.1@{}.json'.format(
            ...
            # load full gt data
            json_path = '/media/6TB_local/SoftTeacher/data/{}/annotations/instances_train_{}.json'.format(args.database, args.database)
