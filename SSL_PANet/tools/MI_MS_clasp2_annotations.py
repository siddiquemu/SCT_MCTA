...
def regress_dets(maskRCNN, proposals=None, im=None, im_name=None, fr=None, pred_score=None, dataset=None,
    ...
    #TODO: proposals might be classified as a wrong class: classification uncertainty
...
def get_masks(maskRCNN, detPB, im=None, vis_img=None, im_name=None, fr=None, pred_score=None, dataset=None,
    ...
    dets_proposals[:, 2:4] = dets_proposals[:, 0:2] + dets_proposals[:, 2:4]
    cls_boxes, cls_segms, class_segms_coarse = regressed_box2mask(maskRCNN, im,
                                    box_proposals=dets_proposals,
                                    timers=timers,
                                    test_aug=0,
                                    soft_nms=0,
    ...
    if det_size > 0:
        ...
        det_class, mask_class, _ = get_box_mask_single_cat(fr, cls_boxes, cls_segms, proposals=detPB, thresh=0,
                                                ...
                                                cls_segms_coarse=class_segms_coarse, img=im, rgr_refine=False)
    else:
        mask_class = []
        det_class = []
...
def rotated_boxes(masks, img_rot=None, angle=None):
    ...
    return np.array(rot_boxes).astype('int')
...
def main(folders, angleSet, dataset_clasp, output_dir, isTrain, imgResultDir, soft_nms=False,
         class_list=None, data_type='gt', save_data=False, saveAugResult=False, apply_cluster_mode=False,
        det_thr=0.5, all_scores=None, regress_cluster=None, vis_annos=False,
         cluster_score_cams=None, database='clasp1', nms_thr=0.3, gpu_id=1, verbose=False):
    """main function"""
    # configure detector
    ...
    # set NMS
    print(f'det_thr: {det_thr}, nms_thr:{nms_thr}')
    ...
    for cam_path in folders:
        ...
        for i, im_name in enumerate(imglist):
            ...
            if fr_num in train_set and (fr_num)%1==0 and fr_num not in test_set:
                pr.enable()
                            ...
                            if (len(dets) > 0):
                                for i, box in enumerate(dets):
                                    ...
                                    if database=='clasp1':
                                        ...
                                        if  box[4]<960 and (box[6] >= 0.5 and box[7] == 1):
                                            if cam_path.split('/')[-1] in ['G_11', 'H_11', 'I_11'] and (box[3] + box[5]) > 550:
                                                detPB.append(box)
                                                maskPB_rot.append(masks_rot[i])
                                                detPB_rot.append(dets_rot[i])
                                            else:
                                        ...
                                        if box[4]<960 and (box[6] >= 0.5 and box[7] != 1):
                                            if cam_path.split('/')[-1] in ['G_9', 'H_9', 'I_9'] and ((box[2] + box[4] / 2) < 1500
                                                    or (box[3] + box[5] / 2) < 700):
                                                detPB.append(box)
                                                maskPB_rot.append(masks_rot[i])
                                                detPB_rot.append(dets_rot[i])

                                            if cam_path.split('/')[-1] in ['G_11', 'H_11', 'I_11']:
                                                detPB.append(box)
                                                maskPB_rot.append(masks_rot[i])
                                                detPB_rot.append(dets_rot[i])
                ...
                if len(detPB) > 0 and saveAugResult:
                    ...
                    if regress_cluster:
                        ...
                        for cl in class_list:
                            ...
                            if len(det_cl[cl]) > 0:
                                det_cl[cl] = regress_dets(maskRCNN, proposals=det_cl[cl], im=im, im_name=os.path.basename(im_name), fr=fr_num,
                    ...
                    if fr_num % 5 == 0:
                        ...
                        show_result = False
                    cluster_score_thr = cluster_score_cams[im_name.split('/')[-3]]
                ...
                if len(det_indexs) > 0 and len(detPB) > 0 and len(cluster_modes)>0 and save_data:
                    ...
                    for theta in [0,90,186,270,354]:#[0, 6,  84, 90, 96,  174, 180, 186, 270, 354]:#angleSet:
                        ...
                        if apply_cluster_mode:
                            ...
                            if semi_supervised and theta == 0 and fr_num in semi_frames:
                                anns_fr = get_frame_anns(coco_clasp, fr_num, catIDs=class_list)
                                fr_det = [ann['bbox'] for ann in anns_fr]
                                fr_mask = [ann['segmentation'] for ann in anns_fr]
                                #update cluster modes from fr_det
                                #cluster_modes = gt2modes(fr_det)
                            else:
                                if verbose:
                                ...
                                fr_det,  fr_mask, masks_0 = get_annos_cluster_mode(maskRCNN, cluster_modes, masks_0,  imgrot=imgrot, class_list=class_list,
                                                                         ...
                                                                         vis_dir=vis_annos_dir, img_fmt='jpg', angle=theta, verbose=verbose)
                        else:
                        ...
                        if len(fr_det)>0:
                            ...
                            for ib,box in enumerate(fr_det):
                                ...
                                if semi_supervised and theta == 0 and fr_num in semi_frames:
                                    bboxfinal = box
                                    segmPolys = fr_mask[ib]
                                    score = 1.
                                    area = anns_fr[ib]['area']
                                    annID = 1000 * int('%06d' % (annIDcount)) + theta
                                    annIDcount += 1
                                    catID = anns_fr[ib]["category_id"]
                                else:
                                    ...
                                    if len(segmPolys)>0:
                                        ...
                                        print(f'ann: Cam: {cam_folder}, Frame: {fr_num}, cat_id {catID}, score: {np.round(score, 2)}, angle: {theta}')
                    fr+=1
...
if __name__ == '__main__':
    ...
    model_path = f'{storage}/PANet_Models'
    ...
    det_thr = 0.5 #clasp2:0.4-0.65, clasp1:0.5
    nms_thr = 0.3
    #model selection
    if data == 'coco':
        ...
        if data_type=='clasp1_augMS':
            ...
            if alpha_loss:
                load_ckpt = model_path + 'clasp2/modified_loss/iter{}/ckpt/model_step19999.pth'.format(exp - 1)
    ...
    # If semi-supervised
    if semi_supervised:
        #load gt json for AppleA_train only
        ...
        coco_clasp, semi_frames = load_clasp_json(json_path, percent=percent_gt)
    ...
    dataset_clasp, all_scores = main(folders, angleSet, dataset_clasp, SaveImgDir, isTrain, imgResultDir,
                                     ...
                                     det_thr, all_scores, regress_cluster, vis_annos, cluster_score_cams,
                                     database, nms_thr=nms_thr, gpu_id=gpu_id, verbose=verbose)
    if save_data:
