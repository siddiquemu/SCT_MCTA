...
#from core.config_flower import cfg
...
class Generalized_RCNN(nn.Module):
    ...
    def _forward(self, data, im_info, roidb=None, **rpn_kwargs):
        ...
        #TODO: Update 'bbox_rois_certainty' and 'mask_rois_certainty' to modify the loss based on instance uncertainty
        #Follow the same procedure like 'box_to-gt_ind-map'
        rpn_ret = self.RPN(blob_conv, im_info, roidb)
        #import pdb
        #pdb.set_trace()
        if not self.training and roidb is not None:
            #print('cluster proposals shape:', roidb.shape)
            rpn_ret['rois'] = roidb.cpu().numpy()

            #roidb = list(map(lambda x: blob_utils.deserialize(x)[0], roidb))
            #rpn_ret = self.RPN(blob_conv, im_info, roidb)
            #print(rpn_ret['rois'].shape)
            #rpn_ret['rois'].shape[1] == precom_prop.shape[1]
            #rpn_ret['rois'] = precom_prop
        # if self.training:
        ...
        if not cfg.MODEL.RPN_ONLY:
            ...
            #predict cls and box from forward net
        ...
        if self.training:
            ...
            #import pdb
            #pdb.set_trace()
            #TODO: How to collect cluster score for batch gt boxes
            if 'bbox_rois_certainty' in rpn_ret.keys():
                loss_cls, loss_bbox, accuracy_cls = fast_rcnn_heads.fast_rcnn_losses(
                    cls_score, bbox_pred, rpn_ret['labels_int32'], rpn_ret['bbox_targets'],
                    rpn_ret['bbox_inside_weights'], rpn_ret['bbox_outside_weights'],
                    instance_certainty=rpn_ret['bbox_rois_certainty'])
            else:
                print('training data rois should have instance certainty either [0,1] or -1')

            return_dict['losses']['loss_cls'] = loss_cls # frame_uncertainty_factor
            return_dict['losses']['loss_bbox'] = loss_bbox
            return_dict['metrics']['accuracy_cls'] = accuracy_cls

            return_dict['instance_scores'] = rpn_ret['bbox_rois_certainty']
            if cfg.MODEL.MASK_ON:
                if getattr(self.Mask_Head, 'SHARE_RES5', False):
                ...
                # mask loss
                if 'bbox_rois_certainty' in rpn_ret.keys():
                    loss_mask = mask_rcnn_heads.mask_rcnn_losses(mask_pred,
                                                                 rpn_ret['masks_int32'],
                                                                 instance_certainty=rpn_ret['mask_rois_certainty'])
                else:
                    print('training data rois should have instance certainty either [0,1] or -1')
            ...
            #pdb.set_trace()
            #return_dict['log_instance_certainty'] = torch.log(log_instance_certainty).sum()
