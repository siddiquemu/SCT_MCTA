...
def mask_rcnn_losses(masks_pred, masks_int32, instance_certainty=None):
    ...
    if -1 not in instance_certainty:
        #print('rotation invariant losses are using to update the mask loss weights..')
        instance_scores = Variable(torch.from_numpy(instance_certainty)).cuda(device_id)
        uncertainty_fcator = (1 / instance_scores).sum(0) / instance_scores.size(0)
        #lm per instance
        #loss = F.binary_cross_entropy_with_logits(masks_pred.view(n_rois, -1), masks_gt, weight, size_average=False)#, reduce=False, reduction='none'
        loss = F.binary_cross_entropy_with_logits(masks_pred.view(n_rois, -1), masks_gt, weight, reduce=False)
        loss = loss.sum(dim=1)
        assert loss.shape[0]==instance_scores.shape[0]
        #pdb.set_trace()
        # TODO: try wlayer = nn.Sigmoid() ; instance_scores = wlayer(instance_scores)
        #wlayer = nn.Sigmoid()
        #instance_scores = wlayer(10*(instance_scores - 0.5))

        #SSL-alpha
        loss = loss*(instance_scores)
        loss = loss.sum(0)
        loss /= weight.sum()

    else:
        print('default losses are using to update the mask loss weights..')
...
#siddique
def mask_rcnn_fcn_head_v1up5convs_gn_adp_ff(dim_in, roi_xform_func, spatial_scale):
    """v1up design: 5 * (conv 3x3), convT 2x2, with GroupNorm"""
    return mask_rcnn_fcn_head_v1upXconvs_gn_adp_ff(
        dim_in, roi_xform_func, spatial_scale, 5
    )
