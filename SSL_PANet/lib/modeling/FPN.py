...
def fpn_rpn_losses(**kwargs):
    ...
    for lvl in range(cfg.FPN.RPN_MIN_LEVEL, cfg.FPN.RPN_MAX_LEVEL + 1):
        ...
        #modify proposals generation losses
        #loss_cls = F.cross_entropy(rpn_cls_logits_fpn, rpn_labels_int32_fpn, reduce=False)
        #assert instance_scores.shape==loss_rpn_cls_fpn.shape
        #SSL-alpha
        #loss_rpn_cls_fpn = loss_rpn_cls_fpn*(instance_scores)
        #loss_rpn_cls_fpn = loss_rpn_cls_fpn.sum(0) / instance_scores.size(0)
        ...
        loss_rpn_bbox_fpn = net_utils.smooth_l1_loss(
            ...
            beta=1/9, instance_certainty=None)
