...
def combined_roidb_for_training(dataset_names, proposal_files, ssl_iter=None):
    """Load and concatenate roidbs for one or more datasets, along with optional
    object proposals. The roidb entries are then prepared for use in training,
    which involves caching certain types of metadata for each roidb entry.
    """
    def get_roidb(dataset_name, proposal_file, ssl_iter):
        ds = JsonDataset(dataset_name, ssl_iter)
        roidb = ds.get_roidb(
            gt=True,
    ...
    print(dataset_names)
    ssl_iters = (ssl_iter,)* len(dataset_names)
    roidbs = [get_roidb(*args) for args in zip(dataset_names, proposal_files, ssl_iters)]

    #create union of multiple datasets
    roidb = roidbs[0]
    for r in roidbs[1:]:
        roidb.extend(r)
