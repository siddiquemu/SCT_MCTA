...
from .dataset_catalog import ANN_FN_FL
from .dataset_catalog import IM_DIR_FL
...
class JsonDataset(object):
    ...
    def __init__(self, name, ssl_iter):
        print(name)
        if ssl_iter>=0 and name in ['clasp1_2021_aug_train_wo_regress']:
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR]\
                                     +'/iter{}/img1_{}'.format(ssl_iter, ssl_iter)
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN] \
                                     + '/iter{}/clasp1_test_aug_{}.json'.format(ssl_iter, ssl_iter)

        elif ssl_iter>=0 and name in ['clasp1_2021_aug_train_score']:
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR]\
                                     +'/iter{}/img1_{}'.format(ssl_iter, ssl_iter)
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN] \
                                     + '/iter{}/clasp1_test_aug_{}.json'.format(ssl_iter, ssl_iter)

        elif ssl_iter>=0 and name in ['clasp1_2021_mixed_aug_train_score']:
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR]\
                                     +'/iter{}/img1_{}_mixed_aug'.format(ssl_iter, ssl_iter)
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN] \
                                     + '/iter{}/clasp1_test_aug_{}_mixed_aug.json'.format(ssl_iter, ssl_iter)

        elif ssl_iter>=0 and name in ['clasp2_2021_mixed_aug_train_score']:
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR]\
                                     +'/iter{}/img1_{}_mixed_aug'.format(ssl_iter, ssl_iter)
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN] \
                                     + '/iter{}/clasp2_test_aug_{}_mixed_aug.json'.format(ssl_iter, ssl_iter)

        if ssl_iter>=0 and name in ['clasp1_2021_aug_train_score_wo_regress']:
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR]\
                                     +'/iter{}/img1_{}'.format(ssl_iter, ssl_iter)
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN] \
                                     + '/iter{}/clasp1_test_aug_{}.json'.format(ssl_iter, ssl_iter)

        elif ssl_iter>=0 and  name in ['clasp2_2021_aug_train_score']:
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR]\
                                     +'/iter{}/img1_{}'.format(ssl_iter, ssl_iter)
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN] \
                                     + '/iter{}/clasp2_test_aug_{}.json'.format(ssl_iter, ssl_iter)

        elif ssl_iter>=0 and  name in ['clasp2_2021_aug_train_score_wo_regress']:
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR]\
                                     +'/iter{}/img1_{}'.format(ssl_iter, ssl_iter)
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN] \
                                     + '/iter{}/clasp2_test_aug_{}.json'.format(ssl_iter, ssl_iter)

        elif ssl_iter>=0 and  name in ['clasp2_2021_aug_train_wo_regress']:
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR]\
                                     +'/iter{}/img1_{}'.format(ssl_iter, ssl_iter)
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN] \
                                     + '/iter{}/clasp2_test_aug_{}.json'.format(ssl_iter, ssl_iter)

        elif ssl_iter>=0 and  name in ['flower_2021_aug_score']:
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR]\
                                     +'/iter{}/img1_{}'.format(ssl_iter, ssl_iter)
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN] \
                                     + '/iter{}/flower_test_aug_{}.json'.format(ssl_iter, ssl_iter)
                                     
        elif ssl_iter>=0 and  name in ['flower_2021_aug_score_old']:
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR_FL]\
                                     +'/iter{}/img1_{}'.format(ssl_iter, ssl_iter)
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN_FL] \
                                     + '/iter{}/AppleA_test_aug_{}.json'.format(ssl_iter, ssl_iter)
        else:
            #for SL model
            DATASETS[name][IM_DIR] = DATASETS[name][IM_DIR]
            DATASETS[name][ANN_FN] = DATASETS[name][ANN_FN]
    ...
    def valid_cached_keys(self):
        ...
        keys = ['boxes', 'segms', 'gt_classes', 'seg_areas', 'gt_overlaps',
                'is_crowd', 'box_to_gt_ind_map', 'instance_certainty']
    ...
    def get_roidb(
        ...
        if gt:
            ...
            cache_filepath = os.path.join(self.cache_path, self.name+'_gt_roidb_iter{}.pkl'.format(self.ssl_iter))
            #avoid cache path search: siddique
            if False: #os.path.exists(cache_filepath) and not cfg.DEBUG
    ...
    def _prep_roidb_entry(self, entry):
        ...
        #instance cluster score
        entry['instance_certainty'] = np.empty((0), dtype=np.float32)
        if self.keypoints is not None:
    ...
    def _add_gt_annotations(self, entry):
        ...
        instance_certainty = np.zeros((num_valid_objs), dtype=entry['instance_certainty'].dtype)
        #import pdb
        #pdb.set_trace()
        if self.keypoints is not None:
        ...
        for ix, obj in enumerate(valid_objs):
            ...
            instance_certainty[ix] = obj['instance_certainty']
        ...
        entry['instance_certainty'] = np.append(entry['instance_certainty'], instance_certainty)
        if self.keypoints is not None:
    ...
    def _add_gt_from_cache(self, roidb, cache_filepath):
        ...
        for entry, cached_entry in zip(roidb, cached_roidb):
            ...
            boxes, segms, gt_classes, seg_areas, gt_overlaps, is_crowd, \
                box_to_gt_ind_map, instance_certainty = values[:8]
            if self.keypoints is not None:
            ...
            entry['instance_certainty'] = np.append(
                entry['instance_certainty'], instance_certainty
            )
            if self.keypoints is not None:
                entry['gt_keypoints'] = np.append(
                    entry['gt_keypoints'], gt_keypoints, axis=0
...
def _merge_proposal_boxes_into_roidb(roidb, box_list):
    ...
    #TODO: Distribute instance certainty using similar gt index pattern in box_to_gt_ind_map
    for i, entry in enumerate(roidb):
        boxes = box_list[i]
        ...
        #import pdb
        #pdb.set_trace()
        box_to_gt_certainty_map = np.ones(
            (num_boxes), dtype=entry['instance_certainty'].dtype
        )
        ...
        if len(gt_inds) > 0:
            ...
            #pdb.set_trace()
            box_to_gt_certainty_map[I] = entry['instance_certainty'][argmaxes[I]]
        ...
        entry['instance_certainty'] = np.append(
            entry['instance_certainty'],
            box_to_gt_certainty_map.astype(
                entry['instance_certainty'].dtype, copy=False
            )
        )
