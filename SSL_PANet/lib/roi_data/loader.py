...
class RoiDataLoader(data.Dataset):
    ...
    def __getitem__(self, index_tuple):
        ...
        #import pdb
        #pdb.set_trace()
        #print('instance certainty in single_db: {}'.format(single_db[0]['instance_certainty']))
        ...
        if self._roidb[index]['need_crop']:
            ...
            if len(valid_inds) < len(boxes):
                for key in ['boxes', 'gt_classes', 'seg_areas', 'gt_overlaps', 'is_crowd',
                            'box_to_gt_ind_map', 'instance_certainty', 'gt_keypoints']:
                    assert 'instance_certainty' in entry
                    if key in entry:
