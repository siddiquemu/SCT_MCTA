...
def get_fast_rcnn_blob_names(is_training=True):
    ...
    if is_training:
        ...
        blob_names += ['bbox_rois_certainty']
    ...
    if is_training and cfg.MODEL.MASK_ON:
        ...
        blob_names += ['mask_rois_certainty']
...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    #pdb.set_trace()
    #batch sample certainties
    sampled_certainties = roidb['instance_certainty'][keep_inds]
    #sampled_certainties = np.ones((sampled_rois.shape[0]), dtype=roidb['instance_certainty'].dtype)
    #for ix, certainty in enumerate(roidb['instance_certainty']):
        #sampled_certainties[roidb['box_to_gt_ind_map'][keep_inds]==ix] = certainty
    assert sampled_certainties.shape[0]==bbox_targets.shape[0] # since certainties will aplly on loss

    # Base Fast R-CNN blobs
    blob_dict = dict(
        labels_int32=sampled_labels.astype(np.int32, copy=False),
        rois=sampled_rois,
        bbox_rois_certainty = sampled_certainties,
    ...
    if cfg.MODEL.MASK_ON:
        roi_data.mask_rcnn.add_mask_rcnn_blobs(blob_dict, sampled_boxes, roidb,
                                               im_scale, batch_idx, sampled_certainties)
