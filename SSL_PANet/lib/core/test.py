...
#cfg.TEST.DETECTIONS_PER_IM = 600

def im_detect_box(model, im, box_proposals=None, timers=None, test_aug=False, soft_nms=False, nms_thr=0.5, score_thr=0.5):
    ...
    if not soft_nms:
        print('debug')
    scores, boxes, cls_boxes = box_results_with_nms_and_limit(scores, boxes, test_aug, soft_nms, nms_thr=nms_thr, score_thr=score_thr)
    timers['misc_bbox'].toc()

    return cls_boxes

def im_detect_regress(model, im, box_proposals=None, timers=None, return_mask=False,
                      test_aug=False, soft_nms=False, nms_thr=0.3, score_thr=0):
    """Process the outputs of model for filtering noisy cluster
    Args:
      model: the network module
      im_data: Pytorch variable. Input batch to the model.
      im_info: Pytorch variable. Input batch to the model.
    ...
    # for evaluating results
    timers['misc_bbox'].tic()
    #Siddique: skip nms when clusters are used as proposals
    #i.e soft_nms=0
    assert box_proposals.shape[0] == boxes.shape[0]==scores.shape[0], \
        'found proposals {}, boxes {}, scores {}'.format(box_proposals.shape, boxes.shape, scores.shape)
    scores, boxes, cls_boxes = box_results_with_nms_and_limit(scores, boxes, test_aug,
                                                              soft_nms, nms_thr=nms_thr, score_thr=score_thr)
    timers['misc_bbox'].toc()

    return cls_boxes

def regressed_box2mask(model, im, box_proposals=None, timers=None, return_coarse=False,
                      test_aug=False, soft_nms=False, nms_thr=0.7, score_thr=0):

    scores, boxes, im_scale, blob_conv = im_detect_bbox(
            model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)

    #Siddique: skip nms when clusters are used as proposals or apply regression for cluster modesx
    #i.e soft_nms=0
    assert box_proposals.shape[0] == boxes.shape[0]==scores.shape[0], \
        'found proposals {}, boxes {}, scores {}'.format(box_proposals.shape, boxes.shape, scores.shape)
    scores, boxes, cls_boxes = box_results_with_nms_and_limit(scores, boxes, test_aug,
    ...
    if return_coarse:
        cls_segms_coarse = coarse_mask(
                cls_boxes, masks
            )
    else:
        cls_segms_coarse = None

    cls_segms = segm_results(cls_boxes, masks, boxes, im.shape[0], im.shape[1])

    return cls_boxes, cls_segms, cls_segms_coarse

def im_detect_all(model, im, box_proposals=None, timers=None, test_aug=False, 
            soft_nms=None, nms_thr=None, score_thr=None, return_coarse=False):
    """Process the outputs of model for testing
    Args:
      model: the network module
      im_data: Pytorch variable. Input batch to the model.
    ...
    # for evaluating results
    timers['misc_bbox'].tic()
    scores, boxes, cls_boxes = box_results_with_nms_and_limit(scores, boxes, test_aug, soft_nms, 
                                                            nms_thr=nms_thr, score_thr=score_thr)
    timers['misc_bbox'].toc()
    ...
    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
        ...
        cls_segms = segm_results(cls_boxes, masks, boxes, im.shape[0], im.shape[1])
        # for getting coarse mask ceate separate function
        if return_coarse:
            cls_segms_coarse = coarse_mask(
                cls_boxes, masks
            )
    ...
    return cls_boxes, cls_segms, cls_keyps, cls_segms_coarse
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
    ...
    if boxes is not None:
        #print('debug')
        inputs['roidb'] = inputs['rois']
        inputs['roidb'] = [torch.from_numpy(inputs['roidb'])]
...
def im_detect_mask(model, im_scale, boxes, blob_conv):
    ...
    if boxes.shape[0] == 0:
        ...
        return pred_masks
    #TODO: add an offset for rectangular box to get better prediction around the bounda
    ...
    #print(f'raw mask shapes: {pred_masks.shape}')
    #pdb.set_trace()
    if cfg.MRCNN.CLS_SPECIFIC_MASK:
        pred_masks = pred_masks.reshape([-1, cfg.MODEL.NUM_CLASSES, M, M])
    else:
        pred_masks = pred_masks.reshape([-1, 1, M, M])
    # if pred_masks.shape[0]>4:
    #     pdb.set_trace()
...
def box_results_with_nms_and_limit(scores, boxes, test_aug, soft_nms, nms_thr=None, score_thr=None):  # NOTE: support single-batch
    ...
    #cfg.TEST.SCORE_THRESH = score_thr
    ...
    for j in range(1, num_classes):
        assert score_thr is not None
        inds = np.where(scores[:, j] > score_thr)[0]
        scores_j = scores[inds, j]
        boxes_j = boxes[inds, j * 4:(j + 1) * 4]
        dets_j = np.hstack((boxes_j, scores_j[:, np.newaxis])).astype(np.float32, copy=False)
        if not soft_nms:
            #return all the predicted boxes during test aug. without soft-NMS
            cls_boxes[j] = dets_j
            #print('skipping soft-nms')
        else:
            ...
            nms_dets, _ = box_utils.soft_nms(
                ...
                overlap_thresh=nms_thr, #default:0.5
                score_thresh=0.0001,#default:0.0001
                method=cfg.TEST.SOFT_NMS.METHOD
            )
            #else:
                #keep = box_utils.nms(dets_j, nms_thr)
...
###### Save Coarse Mask - Siddique #############################################
def coarse_mask(cls_boxes, masks):
    """masks: (instance_ind, num_class, M, M)
    """
    num_classes = cfg.MODEL.NUM_CLASSES
    cls_segms_coarse = [[] for _ in range(num_classes)]

    # To work around an issue with cv2.resize (it seems to automatically pad
    # skip j = 0, because it's the background class
    for j in range(0, num_classes):  # can consider the required classes
        segms_coarse = []
        # save all masks of same classs
        #foreground
        if j>0:
            for mask_ind in range(cls_boxes[j].shape[0]):
                # save object score map 28*28
                segms_coarse.append(masks[mask_ind, j, :, :])

            cls_segms_coarse[j] = np.array(segms_coarse)  # mask object/array grouped in class wise??
        #background
        else:
            cls_segms_coarse[j] = masks[:, j, :, :].reshape((masks.shape[0], masks.shape[2], masks.shape[3]))
        # if masks.shape[0]>4:
        #     pdb.set_trace()

    return cls_segms_coarse
