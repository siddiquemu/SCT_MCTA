...
def run_inference_tuned(
    ...
    def result_getter(load_ckpt,load_detectron,test_net_file, output_dir):
        if is_parent:
            # Parent case:
    ...
    all_results = result_getter(load_ckpt,load_detectron,test_net_file, output_dir)
...
def test_net_on_dataset(
    ...
    if multi_gpu:
        ...
        all_boxes, all_segms, all_keyps = multi_gpu_test_net_on_dataset(
            load_ckpt,load_detectron,test_net_file, dataset_name, proposal_file, num_images, output_dir
        )
    else:
        if multi_gpu:
            iscuda = True
        all_boxes, all_segms, all_keyps = test_net(
            load_ckpt, load_detectron , dataset_name, proposal_file, output_dir,cuda=iscuda, gpu_id=gpu_id
        )
    test_timer.toc()
...
def multi_gpu_test_net_on_dataset(
    ...
    binary = os.path.join(binary_dir, test_net_file + binary_ext)
...
def test_net(
    ...
    model = initialize_model_from_cfg(load_ckpt,load_detectron, cuda=False, gpu_id=gpu_id)
...
def initialize_model_from_cfg(load_ckpt,load_detectron, cuda=False, gpu_id=0):
    ...
    if load_detectron:
        logger.info("loading detectron weights %s", load_detectron)
        load_detectron_weight(model, load_detectron)
