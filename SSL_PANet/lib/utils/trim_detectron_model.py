...
#
def handle_box_mask_head(_d,save_path):
    #cfg.merge_from_file(args.cfg)
    #_d = load_c2_format(cfg, DETECTRON_PATH)
    newdict = _d

    newdict['model'] = removekey(_d['model'],
                                 ['cls_score.bias', 'cls_score.weight', 'bbox_pred.bias', 'bbox_pred.weight','classify.weight','classify.bias'])
    torch.save(newdict, save_path)
