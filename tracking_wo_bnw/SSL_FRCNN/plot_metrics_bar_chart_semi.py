...
for metric in ['F1','Recall', 'Precision', 'AP', 'MODA',  'MODP']:
    ...
    rects2 = plt.bar(index + bar_width*(2-1), person_ssl_alpha, bar_width,
                     alpha=opacity_ours,
                     color='green',
                     label='person_ssl_alpha')

    rects3 = plt.bar(index + bar_width*(3-1) , person_sl, bar_width,
                     alpha=opacity_ours,
                     color='blue',
                     label='person_sl')
    rects4 = plt.bar(index + bar_width*(4-1), person_semi_sl, bar_width,
                    alpha=opacity_ours + 0.3,
                    color='green',
                    label='person_semi_sl')
    ...
    rects6 = plt.bar(index + bar_width*(6-1), bag_ssl_alpha, bar_width,
                     alpha=opacity_ours,
                     color='magenta',
                     label='bag_ssl_alpha')

    rects7 = plt.bar(index + bar_width*(7-1), bag_sl, bar_width,
                     alpha=opacity_ours,
                     color='black',
                     label='bag_sl')
    rects8 = plt.bar(index + bar_width*(8-1), bag_semi_sl, bar_width,
                    alpha=opacity_ours + 0.3,
                    color='magenta',
                    label='bag_semi_sl')
    ...
    plt.legend(('P-baseline', 'P-SSL', 'P-SL', 'P-Semi-SL',
                'B-baseline', 'B-SSL', 'B-SL', 'B-Semi-SL'),
               loc='lower left',ncol=2, prop={'size': 12})

    plt.tight_layout()
    plt.savefig(os.path.join(out_path, 'Det_{}_Semi_SL.png'.format(metric)))
    #plt.show()
