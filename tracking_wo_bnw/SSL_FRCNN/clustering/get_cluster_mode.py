...
from clustering.mean_shift import MeanShift as MS
from scipy.spatial import distance

class Cluster_Mode(object):
    def __init__(self, detPB=None, frame=None, angle_set=None, img=None, out_path=None,
                 save_dets_dict=None, vis=True, nms=True, bw_type='estimated',
                 save_modes=False, cluster_scores_thr=[0,0], feat_dim=4, verbose=False,
                 feature_type='mask', save_scores=None, global_frame=None):
        self.detPB = detPB
        ...
        self.color_remap = [(0, 255, 0), (255, 255, 0)]
        self.frame = frame
        self.img = img
    ...
    def show_active_cluster(self, cluster, class_id=1):
        ...
        for bb in cluster:
            #if (bb[6]>=0.5 and class_id==1) or (bb[6]>=0.5 and class_id!=1):
        ...
        for j in labels_unique:  # cluster wise loop
            if j!=-1:
                ...
                if cluster_prob_score>= score_thr:
                    ...
                    #show cluster modes
                    if self.vis:
                        cv2.putText(self.img, str(score), (int(refined_det[4] ), int(refined_det[5])), 
                                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 4)

                        self.img = cv2.rectangle(self.img, (int(refined_det[4]), int(refined_det[5])), 
                                                (int(refined_det[4] + refined_det[6]), 
                                                int(refined_det[5] + refined_det[7])), color, 4)
