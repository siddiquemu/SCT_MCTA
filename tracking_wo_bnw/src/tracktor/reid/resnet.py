...
class ResNet(models.ResNet):
    def __init__(self, block, layers, output_dim):
        ...
        self.avgpool = nn.AvgPool2d((8,8), stride=1)#(8,4): 128x128
    ...
    def build_crops(self, image, rois):
        res = []
        trans = Compose([ToPILImage(), Resize((256,256)), ToTensor()])
        for r in rois:
            x0 = int(r[0])
            y0 = int(r[1])
            x1 = int(r[2])
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_pretrained_dict(model_zoo.load_url(model_urls['resnet50'], model_dir='/media/siddique/464a1d5c-f3c4-46f5-9dbb-bf729e5df6d61/tracking_wo_bnw/r50_backbone'))
    return model