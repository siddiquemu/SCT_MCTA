...
from .mot_sequence import MOT17Sequence, CLASPSGTsequence
...
class MOTreID(CLASPSGTsequence):#MOT17Sequence
	...
	def __init__(self, seq_name, frame_offset, split, vis_threshold, P, K, max_per_person,
				 crop_H, crop_W, transform, normalize_mean=None, normalize_std=None):

		super(MOTreID, self).__init__(seq_name, vis_threshold=vis_threshold )

		self.P = P
		self.K = K
		self.max_per_person = max_per_person
		#set crop W, H based on camera angles
		self.crop_H = crop_H 
		self.crop_W = crop_W
		# set clasp2-GT params for CLASPSGTsequence class
	...
	def build_samples(self):
		...
		for k,v in tracks.items():
			...
			if l >= self.K:
				...
				print('{} instances for person {}'.format(l, k))
