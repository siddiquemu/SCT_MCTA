...
from .mot_wrapper import CLASP2Wrapper, MOT17Wrapper, MOT19Wrapper, MOT17LOWFPSWrapper, MOT20Wrapper
from .mot_reid_wrapper import MOTreIDWrapper, CLASPreIDWrapper
...
for split in ['train', 'test', 'all', '2', '5', '9', '11', '13', '14']:
    # only FRCNN detections
    name = f'CLASP_{split}'
    print(name)
    _sets[name] = (lambda *args, split=split: CLASP2Wrapper(split, *args))
...
# prepare clasp dataset for reID
for split in ['train_reid', 'small_val', 'small_train']:
    name = f'clasp1_{split}'
    _sets[name] = (lambda *args, split=split: CLASPreIDWrapper(split, *args))
