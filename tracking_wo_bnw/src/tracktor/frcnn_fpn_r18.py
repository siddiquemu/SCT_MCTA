...
if __name__=='__main__':
    ...
    #R18: 49 layers, R34: 65 layers, R50: 82 layers, R101: 133 layers
    frcnn_r18 = FRCNN_FPN(num_classes=2,
                        box_nms_thresh=0.5,
                        backbone_type=backbone,
                        class_id=1)
    print(frcnn_r18)
    i=0
    for param in frcnn_r18.parameters():
        ...
        i+=1
    st_time = time.time()
    x = torch.randn((10, 3, 800, 1200)).cuda()
    y = frcnn_r18(x)
    print(f'{backbone} prediction time before torch2trt fp32: {time.time() - st_time}')