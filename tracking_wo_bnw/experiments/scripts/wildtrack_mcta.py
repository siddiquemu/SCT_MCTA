...
import tracemalloc
import sys
sys.path.insert(0,'/media/siddique/464a1d5c-f3c4-46f5-9dbb-bf729e5df6d61/tracking_wo_bnw/experiments/experiments/cfgs')
...
from merge_batch_tracklets import SCT_ReID
...
class SCT_Clasp(object):
    ...
    def init_det_model(self):
        ...
        if self.init_flags['dataset']=='logan':
            self.obj_detect.load_state_dict(torch.load(self._config['tracktor']['obj_detect_logan_model'],
    ...
    def init_online_dataloader(self, dataset, img_path, fr_num, data_loc):
        ...
        '''
        if data_loc == 'logan':
            path_i = os.path.join(img_path, 'C{}/img1/{:05d}.jpg'.format(self.cam, fr_num))
    ...
    def test_SCT(self, tracktor=None, start_frame=1, fr_step = 1, dataset=None,
        ...
        while num_frame <= last_frame:
            ...
            if num_frame % self.init_flags['fr_step'] == 0:
                cam_results = tracktor.get_results()
                if self.init_flags['SCT_ReID']:
                    print(self.global_track_start_time.keys())
                    #issue: confusion between prev. lost track and new track
                    # need to predict the missing frame location using linear motion model
                    # interpolate lost racke last position until current frame
                    sct_reid = SCT_ReID(id_init_map=self.global_track_start_time,
                                        all_tracklets=cam_results, curr_frame=num_frame,
                                        t_thr=10, d_thr=150, reid_map=self.init_flags['sct_reid_map'],
                                        new_id=self.new_id)
                    self.init_flags['sct_reid_map'] = sct_reid.match_tl()
                    print(self.init_flags['sct_reid_map'])

                plot_scanned_track(num_frame, cam_results, frame['img_path'], save_dir,cam=self.cam, reid_map=self.init_flags['sct_reid_map'])
            num_frame += 1
    ...
    def test_SCD(self, tracktor=None, start_frame=1, fr_step=1, dataset=None,
        ...
        while num_frame <= last_frame:
            ...
            if det_pos.nelement()>0 and num_frame % self.init_flags['fr_step'] == 0:
                plot_dets(num_frame, camfr_results, frame['img_path'], save_dir,cam=self.cam, labels=1)
            num_frame += fr_step
...
if __name__ == '__main__':
    ...
    init_params = {}
    init_params['dataset'] = 'logan' #'wild-track'
    ...
    if init_params['dataset'] == 'logan':
        ...
        init_params['img_path'] = benchmark #os.path.join(benchmark, init_params['dataset'], 'img1')
        init_params['batch_end'] = 2990
        init_params['fr_step'] = 1
        init_params['output_dir'] = storage + 'tracking_wo_bnw/output/tracktor_logan/SCT'
        #for det
        #init_params['output_dir'] = storage+'tracking_wo_bnw/output/tracktor-det-logan'
        init_params['skip_nms'] = 1

    init_params['sct_reid_map'] = {}
    init_params['SCT_ReID'] = 1
