...
from tracktor.utils import interpolate, plot_sequence, \
    plot_scanned_track, filter_tracklets_app, delete_all, get_mot_accum, \
    evaluate_mot_accums
import pdb
import logging
from read_config import Configuration
#from MCTA.MCTA_Online import MCTA
from MCTA.MCTA_PVD_app import MCTA
...
class SCT_Clasp(object):
    ...
    def init_det_model(self):
        ...
        if self.init_flags['backbone'] == 'ResNet50FPN':
            #train camera specific models: 330 will fit in SSL
            if self.cam in [360]:
                ...
                self.obj_detect.load_state_dict(torch.load(self._config['tracktor']['obj_detect_model_tso_iter10'],
                                                   map_location=lambda storage, loc: storage))
...
if __name__ == '__main__':
    ...
    init_params['batch_start'] = 2001#1#1801#1 #3701
