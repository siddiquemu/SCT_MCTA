...
class EvaluateMOT(object):
    ...
    def sct_track_eval_table(self, summary=None, eval_type='mcta_overall'):
        ...
        assert 'OVERALL' in summary.index.values, "if overall is not found check the pandas==0.25.3"
        
        for data_name, mot_name in mm.io.motchallenge_metric_names.items():
            #We can skip any measures based on the table requirements
            # -1 is for skipping the overall from the summary
    ...
    def compare_gt_track2D(self, gt, track, fr=None, result_name=None):
        ...
        self.img = cv2.imread(os.path.join(self.img_path, self.gt_folder_map[result_name], 'img1/{:06d}.png'.format(int(fr))))
...
def get_dirs(database, storage, isMCTA, det_model, dist_metric):
    ...
    if database == 'clasp1':
        #full set
        gt_path = storage + 'tracking_wo_bnw/data/CLASP1/gt_mc'
        gt_img_path = storage + 'tracking_wo_bnw/data/CLASP1/train_gt'
...
def get_gt_stats(gt, seq):
    print('              ')
    print('######## CLASP1 MCTA Person GT Stats: {} ########'.format(seq))
...
if __name__ == '__main__':
    ...
    database = 'clasp1'
    det_model = 'SSL_alpha'
    dist_metric = 'frechet' #'hausdorff' #'frechet'
    class_id = 1
    vis=0
    cams=[9, 11]
    #to report eval
    ...
    meta_map = {'exp9a':'A','exp10a':'D', 'exp7a':'C', 'exp6a':'B', 'exp5a':'E'}
    if isMCTA:
        #fr_rate_factor = {'A':1, 'D':1, 'C':1, 'B':1, 'E':1 }#, 'B':1, 'C':1, 'D':1, 'E':1}
        #offset = {'A':0, 'D':0, 'C':0,  'B':0, 'E':0}#, 'B':30, 'C':30, 'D':30, 'E':0}
        fr_rate_factor = {'A':30, 'D':15, 'C':15,  'B':15, 'E':3}
        offset = {'A':15, 'D':30, 'C':30,  'B':30, 'E':4}#, 'B':30, 'C':30, 'D':30, 'E':0}
    else:
        fr_rate_factor = {'A':30, 'D':15, 'C':15,  'B':15, 'E':3}
        offset = {'A':15, 'D':30, 'C':30,  'B':30, 'E':4}#, 'B':30, 'C':30, 'D':30, 'E':0}
    ...
    for exp_name, d_name in meta_map.items():
        ...
        gt_folders = glob.glob(gt_path + '/{}/*'.format(exp_name))
        gt_folders.sort(key=lambda f: int(''.join(filter(str.isdigit, str(f)))))

        if isMCTA:
            track_dir = result_path + '/{}*.txt'.format(exp_name.lower())
            assert os.path.exists(result_path), '{} not exists'.format(result_path)
            tr_files = glob.glob(track_dir)

        else:
            # SCT
            print('evaluate SCT on MC annotations')
            tr_files = glob.glob(result_path + '/{}*.txt'.format(exp_name.lower()))
        ...
        print('gt:{}, tr: {}'.format(gt_folders, tr_files))
        ind_gt = 0
        for gt_file, tr_file in zip(gt_folders, tr_files):
            print('gt:{}, tr: {}'.format(os.path.basename(gt_file), os.path.basename(tr_file)))
    ...
    #issue: True for generate_overall is not working:
    #This is a problem with the version of pandas used by py-motmetrics. If you do the following, it will work fine.
    #pip install pandas==0.25.3
    # i use local PANet environment to get overall
