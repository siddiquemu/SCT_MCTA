...
class Projection(object):
    def __init__(self, gt_path, out_path, images_path):
    ...
    def images_getter(self, cam):
        if os.path.isdir(self.images_path):
            ...
            print('synchronized image path is not correct')
        return im_list
    ...
    def projectCentroid(self, center, cam_params, cam=2):
        # https://stackoverflow.com/questions/12299870/computing-x-y-coordinate-3d-from-image-point
        centroid = center
        box_height = 200
        bos_pos = 30
        ...
        projb, _ = cv2.projectPoints(wpoints.astype('float'), cam_params['C1']['rvec'], cam_params['C1']['tvec'],
        ...
        proj, _ = cv2.projectPoints(wpoint_center.reshape(1,3), cam_params['C1']['rvec'], cam_params['C1']['tvec'],
                                    cam_params['C1']['A'], cam_params['C1']['dist_coeff'])
        # pdb.set_trace()
        # if cam in [9,2]:
