...
class MCTA(object):
            ...
            if (fr) % vis_rate == 0:
                ...
                if save_imgs:
                    ...
                    if save_imgs:
                        ...
                        imgC9C2 = cv2.vconcat([imgC9, imgC2, blankImg])
                        imgC11C5 = cv2.vconcat([imgC11, imgC5, imgC4])
                        ...
                        final_img = cv2.resize(final_img, (int(1 * 1080), 720), interpolation=cv2.INTER_AREA)#1.5
    ...
    def generate_global_graph(self,cacp, mt):
        ...
        if len(mt)>0: #TODO: G_mc only for associated tracks or for all in unique track list??
            ...
            self.G_mc = merge_edges(mt, self.G_mc, batch_ids=cam2cam_params['batch_ids_list'], cam_batch_ids=self.cam_trklts_id, ca=ca,cp=cp)
    ...
    def get_global_matches(self, vis_pair):
        ...
        cam_track_idx = {keys: 0 for keys in range(0, len(cam_list))}
        global_track_results3D = {'frame': [], 'id': [], 'Cx': [], 'Y2': [], 'w': [], 'h': []}
        global_track_results2D = {'frame': [], 'id': [], 'x': [], 'y': [], 'w': [], 'h': [], 'cam': []}
        for c in cam_list:
            self.MCparams[c] = self.dafault_params(self.img_path,
                                         self.pathTrackers,
                                         self.cam_projections,
                                         isDistorted=self.DistCorrect,
                                         cam=c, out_dir=self.out_path)

            self.MC_tracks[c] = get_projected_tracks(self.MCparams[c],
                                                projection_class=self.cam_projections,
                                                isDistorted=self.DistCorrect,
                                                isseqID=False,
                                                isSCT=False,
            ...
            combined_tracks3D.append(MC_tracks[c]['3D_tracks'])
            combined_tracks2D.append(MC_tracks[c]['2D_tracks'])
            # keep track the tracklet index in affinity concatenation
            MC_tracks[c]['track_idx'] = cam_track_idx[c - 1] + np.array(range(len(combined_tracks3D[c - 1])))
            cam_track_idx[c] = cam_track_idx[c - 1] + len(combined_tracks3D[c - 1])

        self.combined_tracks3D, _ = expand_from_temporal_list(box_all=self.combined_tracks3D, mask_all=None)
        for i in self.cam_list:
            print('track_idx {}'.format(self.MC_tracks[i]['track_idx']))
            print('cam {}'.format(self.combined_tracks3D[MC_tracks[i]['track_idx']][0][-1][-1]))
        # TODO: optimum threshold
        # hausdorf: 75, frechet: 150
        self.global_tracks3D, self.cluster_map = CM_global_tracks_mcsystems(self.combined_tracks3D, cam_net=self.cam_net,
                                                                  params=self.MC_tracks,
                                                                  max_dist=100, metric_type='frechet')

        colors = ["#" + ''.join([random.choice('0123456789ABCDEF') for j in range(6)]) for i in
                  range(int(max(self.cluster_map.keys())) + 10)]
        # global_tracks = CM_global_tracks(MC_tracks, max_dist=100)
        # global_tracks,_ =  expand_from_temporal_list(box_all=global_tracks, mask_30=None)
        # globa 3D tracks in GT is 313
        # colors = get_cmap(len(global_tracks), name='hsv')
        if eval_plane == '2D':
            combined_tracks2D, _ = expand_from_temporal_list(box_all=self.combined_tracks2D, mask_all=None)
            self.global_track_results = self.get_sequential_results_2D(self.cluster_map,
                                                                       self.combined_tracks2D,
                                                                       self.global_track_results2D,
                                                                       img_dir=self.img_path,
                                                                       out_dir=self.out_path + '/global_tracks',
                                                                       fr_start=fr_start, fr_end=fr_end, colors=colors)
        else:
            self.global_track_results = self.get_sequential_results_3D(self.global_tracks3D, self.global_track_results3D, out_dir=self.out_path + '/global_tracks',
                                      fr_start=fr_start, fr_end=fr_end, colors=colors)
