...
def associate_tracklets_DFS_MC(params, cam2cam, isDistorted=False, max_dist=400):
        ...
        for tracklet2 in params['PAproj']:
            ...
            if i != j and i < params['trkltFamilySizeP'] and j >= params['trkltFamilySizeP'] \
                ...
                # Search for the overlapping portions of multicamera tracklets
                    ...
                    final_tracklet2 = tracklet2[mask2][:, featIndx]
                    final_tracklet2[:, 1:5] = final_tracklet2[:, 1:5] / [1920.0, 1080.0, 50.0, 50.0]
                    if cam2cam in ['H5to2','H9to2','H13to14']:
                        d_h = directed_hausdorff(final_tracklet1[:, 1:3], final_tracklet2[:, 1:3])[0]
...
def plot_trajectories_MC(labelsC9,
    ...
    assert len(MCparams['H13to14']['PAorg']) == len(labelsC13) + len(labelsC14)
    # BGR
    # magenta = (255,0,255)
    # yellow = (0,255,255)
    # green = (0,255,0)
    ...
    for fr in range(fr_start, fr_end):
        ...
        if (fr) % vis_rate == 0:  # 15 for 6A,7A,.. 3 for 5A, 5B.. 30 for 9A
            ...
            imgC13 = cv2.imread('{}{:06d}.png'.format(MCparams[cam2cam['C11C13']]['AImgsPath'],
                                                      fr + (-44)))
            imgC14 = cv2.imread('{}{:06d}.png'.format(MCparams[cam2cam['C13C14']]['PImgsPath'],
                                                      fr + (-42)))
            ...
            for bb in MCparams['H5to11']['PAorg'][:len(labelsC11)]:
                ...
                for i in range(1, len(bb)):
                    if fr_start <= bb[i, 0] <= fr + 2:
                        ...
                        if metrics:
                            # TODO: Think about how to show this only at the last detection in the current frame
                            if bb[i, 0] == fr + 2:# and labelsC11[c] in [27,18,14,16,22] :
            ...
            for bb in MCparams['H11to13']['PAorg'][len(labelsC11):]:
                ...
                for i in range(1, len(bb)):
                    if fr_start <= bb[i, 0] <= fr - 44:
                        ...
                        if metrics:
                            # TODO: Think about how to show this only at the last detection in the current frame
                            if bb[i, 0] == fr - 44: #and labelsC13[c] in [27,18,14,16,22]:
            ...
            # show bbox on C14
            c = 0
            for bb in MCparams['H13to14']['PAorg'][:len(labelsC14)]:
                # TODO: Set the colors of the rectangles
                for i in range(1, len(bb)):
                    if fr_start <= bb[i, 0] <= fr - 42:
                        # image = cv2.arrowedLine(image, start_point, end_point, color, thickness, tipLength = 0.5)
                        if save_imgs and motion and bb[i, 0] % 5 == 0:
                            # 2d velocity plot
                            xx = int(
                        ...
                        if metrics:
                            # TODO: Think about how to show this only at the last detection in the current frame
                            if bb[i, 0] == fr - 42: #and labelsC14[c] in [27,18,14,16,22]:
                                ...
                                if isDistorted:
                                    ...
                                    cxy_undist = np.copy(bb[i, 1:3] + bb[i, 3:5] / 2.0).reshape(1, 2)
                                # show track id: undistorted centers
                                cv2.putText(imgC14, 'P{}'.format(labelsC14[c]),
                                            (int(cxy_undist[0, 0]), int(cxy_undist[0, 1])),
                                            cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), 5, cv2.LINE_AA)
                c = c + 1
            # translate image
    ...
    for fr in range(fr_start, fr_end):
        if (fr) % vis_rate == 0:
            ...
            if cam2cam == 'H13to14':
                ...
                final_img = cv2.hconcat([img, img2])  # img: c11, img2: c13
...
def dafault_params(cam2cam, experiment, cams, mhtTrackers):
    ...
    if cam2cam == 'H9to2':
        ...
        param['d_th'] = 0.4  #multi-task cost: 0.4, only loc: 0.25, loc+velocity 4d hd: 0.4
    ...
    if cam2cam == 'H5to2':
        ...
        param['d_th'] = 0.2  #multi-task cost: 0.4, only loc: 0.2, loc+velocity 4d hd: 0.4
        param['primary'] = '/home/siddique/Desktop/NEU_Data/May_Demo_Trackers/exp2-train/cam02exp2.mp4testTracksv2.txt'
        # mhtTrackers + 'cam{:02d}{}PersonMASK_30FPSrot.txt'.format(cams['C2'], experiment)#cam2: test_aug: v1
    ...
    if cam2cam == 'H5to11':
        ...
        param['Aid'] = 5
        # C11: pri, C5: aux
        param['tOffsetPstart'] = 25  # 11 delayed 5 by 23: 11>1 == 5>24
   ...
   23.8510  508.6742    1.0000'''
        ...
        param['fr_offsetPVis'] = -25
        param['fr_offsetAVis'] = 0  ## T_c2 - T_c11 = offset
        param['H'] = np.transpose(param['H'])
        # param['H'] = np.linalg.inv(param['H'])
        param['d_th'] = 0.6  # 0.17#0.17#0.45#200 # multi-task cost: 0.45, only loc: 0.17, loc+velocity 4d hd: 0.4
    ...
    if cam2cam == 'H5to13':
        ...
        param['Aid'] = 5
        # C11: pri, C5: aux
        param['tOffsetPstart'] = 70  # 11 delayed 5 by 23: 13>1 == 5>71
        param['tOffsetAstart'] = 0  #
        # similarity based homography: TODO: compute homography using undistorted image
        # H5to11
        '''
        ...
        '''
        param['H'] = [[0.93, -0.089, 0.00],
                      [0.089, 0.93, 0.00],
                      [792.6, 516.32, 1.00]]

        param['fr_offsetPVis'] = -70
        param['fr_offsetAVis'] = 0  ## T_c2 - T_c11 = offset
        param['H'] = np.transpose(param['H'])
        # param['H'] = np.linalg.inv(param['H'])
        param['d_th'] = 0.6  # 0.17#0.17#0.45#200 # multi-task cost: 0.45, only loc: 0.17, loc+velocity 4d hd: 0.4
        param['primary'] = '/home/siddique/Desktop/NEU_Data/May_Demo_Trackers/exp2-train/cam13exp2.mp4testTracks.txt'
        # mhtTrackers + 'cam{:02d}{}PersonMASK_30FPSrot.txt'.format(cams['C2'], experiment)#cam2: test_aug: v1
        param['auxiliary'] = '/home/siddique/Desktop/NEU_Data/May_Demo_Trackers/exp2-train/cam05exp2.mp4testTracks.txt'
        # mhtTrackers + 'cam{:02d}{}PersonMASK_30FPSrot.txt'.format(cams['C9'], experiment)#cam9-test_aug:v2exp1
        ...
        # param['finalTrackFileC2'] = 'cam02exp2_MCTA.txt'
        param['PImgsPath'] = benchmark + experiment + '/imgs/cam{:02d}'.format(cams[
                                                                                   'C13']) + experiment + '.mp4/'  # benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C2']) + 'exp1' + '.mp4/'
        # benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C2']) + tracklet_exp + '.mp4/'
        param['AImgsPath'] = benchmark + experiment + '/imgs/cam{:02d}'.format(cams[
                                                                                   'C5']) + experiment + '.mp4/'  # benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C9']) + 'exp1' + '.mp4/'
        # benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C9']) + tracklet_exp + '.mp4/'
        param['outPath'] = benchmark + experiment + '/Results/MCTA/vis/cam{}to{}/'.format(cams['C5'], cams['C13'])
...
def get_cam2cam_matchedTrklt(params, cam2cam, isDistorted=True, isseqID=False):
    ...
    mht_tracklets, totalTrack = form_tracklets(trackers, fr_startP, fr_endP, tracker_min_size=trklt_size,
                                               t_thr=30, d_thr=50, single_cam_association=isSCA).tracker_mht()
    # list C2 tracklets
    ...
    edges = get_edges(params, mt)

    return mt, label_map_update, params, min(fr_startP, fr_startA), max(fr_endP, fr_endA)
...
def split_edges(mt,G_mc,cp_factor, ca_factor, cp, ca):
    # to keep the order of the trklt index sequential
    #labelPrimary = {keys: [] for keys in range(cp_factor, cp_factor+MC_trklt['trkltFamilySizeP'])}
    #labelAuxiliary = {keys: [] for keys in range(ca_factor+MC_trklt['trkltFamilySizeP'],
                                                 #ca_factor + MC_trklt['trkltFamilySizeP']+MC_trklt['trkltFamilySizeA'])}
    for (c, tStampComnMin, k, l) in mt:
        G_mc[k+cp_factor].append(l+ca_factor)
        G_mc[l+ca_factor].append(k+cp_factor)
        print 'pairwise association k {} l {}'.format(k, l)
        print 'multi-camera system association k {} l {}'.format(k+cp_factor, l+ca_factor)
...
def reform_keys(curr_aux=None, position_factor=None):
    # diff_factor =  prev_aux.keys()[0] - curr_aux.keys()[0]
    new_key = curr_aux.keys()[0]
    # TODO: diff_factor may be +ve or -ve
    reformed_prev_aux = collections.OrderedDict()
...
if __name__ == "__main__":
    ...
    cp_index_factor = {}
    ca_index_factor = {}
    for cp in camP_Q.keys():
        Auxs = camP_Q[cp]
        for ca in Auxs:
            cacp = 'C'+str(ca)+'C'+str(cp)
            MCparams[cam2cam[cacp]] = dafault_params(cam2cam[cacp], experiment, cams, mhtTrackers)
            mt, label_map_update, MC_trklt_C2C, fr_start, fr_end = get_cam2cam_matchedTrklt(MCparams[cam2cam[cacp]],
                                                                                                cam2cam[cacp],
                                                                                                isDistorted=DistCorrect,                                                                                       isseqID=True)
            #save camera pair graph
            MCparams[cacp+'_graph'] = mt

            # prepare traklets info for mc system
            cam2cam_params = MCparams[cam2cam[cacp]]
            if cp not in cp_visited:
                prim_list_trklts.append(cam2cam_params['PAproj'][:cam2cam_params['trkltFamilySizeP']])
                cp_visited.append(cp)
                cp_index_factor[cp] = cam2cam_params['trkltFamilySizeP']
            if ca not in ca_visited:
                aux_list_trklts.append(cam2cam_params['PAproj'][cam2cam_params['trkltFamilySizeP']:])
                ca_visited.append(ca)
                ca_index_factor[ca] = cam2cam_params['trkltFamilySizeA']


    prim_list_trklts,_ =  expand_from_temporal_list(box_all=prim_list_trklts, mask_30=None)
    aux_list_trklts,_ = expand_from_temporal_list(box_all=aux_list_trklts, mask_30=None)
    mc_all_trklts = np.append(prim_list_trklts,aux_list_trklts)


    # merge pairwise graph to single graph
    camP_Q = {2:[9,5],11:[5,13],14:[13]}
    ca_visited = []
    cp_visited = []
    cp_factor = {}
    ca_factor = {}
    #define empty multi-camera single graph
    G_mc = {keys: [] for keys in range(len(mc_all_trklts))}
    for i, cp in enumerate(camP_Q.keys()):
        Auxs = camP_Q[cp]
        for j, ca in enumerate(Auxs):
            cacp = 'C'+str(ca)+'C'+str(cp)
            #save camera pair graph
            mt = MCparams[cacp+'_graph']
            cam2cam_params = MCparams[cam2cam[cacp]]

            # get factors for individual camera position in G_mc
            print('ca {} cp {}'.format(ca,cp))
            if cp == 2:
                cp_factor[cp] = 0
            if cp == 11:
                cp_factor[cp] = cp_factor[2] + cp_index_factor[2]
            if cp == 14:
                cp_factor[cp] = cp_factor[11] + cp_index_factor[11]

            if ca == 9:
                ca_factor[ca] = sum(cp_index_factor.values()) - cp_index_factor[cp]
            if ca==5:
                ca_factor[ca] = sum(cp_index_factor.values()) - cp_index_factor[cp]
                ca_factor[ca] = ca_factor[ca] + ca_index_factor[9]
            if ca==13:
                ca_factor[ca] = sum(cp_index_factor.values()) - cp_index_factor[cp]
                ca_factor[ca] = ca_factor[ca] + ca_index_factor[5] + ca_index_factor[9]

            #update system graph from pair association (generated from Hungarian)
            G_mc = split_edges(mt, G_mc, cp_factor[cp], ca_factor[ca], cp, ca)
    #apply DFS on ms system graph
