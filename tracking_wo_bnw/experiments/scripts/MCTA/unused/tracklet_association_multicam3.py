...
def associate_tracklets_DFS_MC(cams, MC_tracklets, isDistorted=False, max_dist=400):
    ...
    cost = np.ones((len(MC_tracklets['C2C9projC5proj']), len(MC_tracklets['C2C9projC5proj'])))*np.inf
    tilde_t_pa = np.ones((len(MC_tracklets['C2C9projC5proj']), len(MC_tracklets['C2C9projC5proj'])),dtype='float')*np.inf
    i = 0
    matches = []
    for tracklet1 in MC_tracklets['C2C9projC5proj']:
        j = 0
        for tracklet2 in MC_tracklets['C2C9projC5proj']:
            print('Processing tracklets {} and {}'.format(i, j))
            #condition to check multicamera trackets have any overlap
            if i!=j and i<MC_tracklets['trkltFamilySizeC2'] and j>=MC_tracklets['trkltFamilySizeC2'] \
    ...
    while len(cost[cost<=max_dist])>0:
        ...
        for i in idx:
            ...
            indx_t_a = np.where(cost[row_ind[i], :] <= max_dist)[0]
            if len(indx_t_a)>0:
                T_a = MC_tracklets['C2C9projC5proj'][indx_t_a]
                isIntersect = [len(set(MC_tracklets['C2C9projC5proj'][col_ind[i]][:,0]).intersection(t_a[:,0]))>0
                               for t_a in T_a]
                cost[row_ind[i], indx_t_a[isIntersect]] = np.inf
            ...
            indx_t_p = np.where(cost[:, col_ind[i]] <= max_dist)[0]
            if len(indx_t_p)>0:
                T_p = MC_tracklets['C2C9projC5proj'][indx_t_p]
                isIntersect = [len(set(MC_tracklets['C2C9projC5proj'][row_ind[i]][:, 0]).intersection(t_p[:, 0])) > 0
                               for t_p in T_p]
                cost[indx_t_p[isIntersect], col_ind[i]] = np.inf
...
def plot_trajectories_MC(label_map, fr_start, fr_end, MC_tracklets, MCparams, cam2cam, cams, vis_rate=30,
    ...
    assert len(label_map.keys())==len(MC_tracklets['C2C9C5org'])==len(MC_tracklets['C2C9projC5proj'])== \
    ...
    labelCam5 = dict(list(label_map.items())[-MC_tracklets['trkltFamilySizeC5']::])
    #---->>>
    ...
    for fr in range(fr_start, fr_end):
        ...
        if (fr)%vis_rate==0: #15 for 6A,7A,.. 3 for 5A, 5B.. 30 for 9A
            ...
            for i_p,bb in enumerate(MC_tracklets['C2C9C5org'][:MC_tracklets['trkltFamilySizeC2']]):
                # To show the association with the projections (tracklets2)
                for i in range(1, len(bb)):
                    if fr_start <= bb[i, 0] <= fr:
                        if save_imgs and motion and bb[i, 0]%5==0:
            ...
            for bb in MC_tracklets['C2C9projC5proj'][MC_tracklets['trkltFamilySizeC2']:
                                                    MC_tracklets['trkltFamilySizeC2']
                                                    +MC_tracklets['trkltFamilySizeC9']]:
                # TODO: Set the colors of the rectangles
                for i in range(1, len(bb)):
            ...
            for bb in MC_tracklets['C2C9projC5proj'][-MC_tracklets['trkltFamilySizeC5']::]:
                # TODO: Set the colors of the rectangles
                for i in range(1, len(bb)):
                    if fr_start <= bb[i, 0] <= fr:
                        # image = cv2.arrowedLine(image, start_point, end_point, color, thickness, tipLength = 0.5)
            ...
            c = MC_tracklets['trkltFamilySizeC2']
            # tracks9 = tracks[C2:C2+C9]
            for bb in MC_tracklets['C2C9C5org'][MC_tracklets['trkltFamilySizeC2']:
                                                 MC_tracklets['trkltFamilySizeC2']
                                                +MC_tracklets['trkltFamilySizeC9']]:
                # TODO: Set the colors of the rectangles
                for i in range(1, len(bb)):
            ...
            # show bbox on C5
            c = MC_tracklets['trkltFamilySizeC2']+MC_tracklets['trkltFamilySizeC9']
            for bb in MC_tracklets['C2C9C5org'][-MC_tracklets['trkltFamilySizeC5']::]:
                # TODO: Set the colors of the rectangles
                for i in range(1, len(bb)):
                    if fr_start <= bb[i, 0] <= fr:
                        #image = cv2.arrowedLine(image, start_point, end_point, color, thickness, tipLength = 0.5)
            ...
            imgC5blank = cv2.vconcat([blankImg, imgC5])
            final_img = cv2.hconcat([imgC5blank,imgC9C2]) # img: c4, img2: c2
...
def dafault_params(cam2cam, experiment, cams, mhtTrackers):
    ...
    if cam2cam == 'H5to11' and experiment == 'exp1':
        ...
        param['H5to11'] = [[0.9704, 0.1027, 0.00],
                           [-0.1027, 0.9704, 0.00],
                           [37.211, 514.028, 1.00]]
        param['tOffstC5fromC11'] = -89-40 #synced wrt C2: Tc11 lag Tc5: 89; Tc2 lag TC5 :40
        param['tOffstC11fromC5'] = 0 #synced wrt C2 but wrt C5 is -89
        param['fr_offsetC5Vis'] = 40
        param['fr_offsetC11Vis'] =  89+40   ## T_c2 - T_c11 = offset
...
if __name__ == "__main__":
    ...
    if dataset =='clasp2demo':
        benchmark = '/media/siddique/RemoteServer/LabFiles/CLASP2/2019_10_24/test_data/exp1/'
        mhtTrackers = '/media/siddique/RemoteServer/LabFiles/CLASP2/2019_10_24/test_data/exp1/Results/exp1-test'
    ...
    #mt, tCommonPA = associate_tracklets_DFS(cams['C2'], cam2to1_tracklets, tracklets[0], tracklet_2,
                                            #active_trklt, isDistorted=isDistCorrect, max_dist=MCparams[cam2cam]['d_th'])
    ...
    edges = {keys: [] for keys in range(len(MC_tracklets['C2C9projC5proj']))}
    ...
    label_map = {keys: [] for keys in range(len(MC_tracklets['C2C9projC5proj']))}
    tStampStart = {keys: [] for keys in range(len(MC_tracklets['C2C9projC5proj']))}
    for label_tp,tklt in enumerate(MC_tracklets['C2C9projC5proj']):
        solnPath  = dfs(edges,label_tp,[])
        label_map[label_tp].append(solnPath)
        for tStampInd in solnPath:
            tStamp = MC_tracklets['C2C9projC5proj'][tStampInd][:,0].min()
    ...
    label_map_update = {keys: [] for keys in range(len(MC_tracklets['C2C9projC5proj']))}
