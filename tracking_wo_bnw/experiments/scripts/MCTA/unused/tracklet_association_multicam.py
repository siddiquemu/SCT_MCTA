...
def associate_tracklets_DFS_MC(params, isDistorted=False, max_dist=400):
    ...
    for tracklet1 in params['PAproj']:
        ...
        for tracklet2 in params['PAproj']:
            #condition to check multicamera trackets have any overlap
            if i!=j and i<params['trkltFamilySizeP'] and j>=params['trkltFamilySizeA']\
...
def plot_trajectories_MC(labelsC9,
    ...
    assert len(MCparams['H2to5']['PAorg']) == len(labelsC2) + len(labelsC5)
        ...
        if (fr)%vis_rate==0: #15 for 6A,7A,.. 3 for 5A, 5B.. 30 for 9A
            try:
                ...
                imgC2 = cv2.imread('{}{:06d}.png'.format(MCparams[cam2cam['C9C2']]['PImgsPath'],
                                                         fr+MCparams[cam2cam['C9C2']]['fr_offsetPVis']))
                ...
                imgC9 = cv2.imread('{}{:06d}.png'.format(MCparams[cam2cam['C9C2']]['AImgsPath'],
                                                         fr+MCparams[cam2cam['C9C2']]['fr_offsetAVis']))
                imgC5 = cv2.imread('{}{:06d}.png'.format(MCparams[cam2cam['C2C5']]['AImgsPath'],
                                                         fr+MCparams[cam2cam['C2C5']]['fr_offsetAVis']))
                imgC11 = cv2.imread('{}{:06d}.png'.format(MCparams[cam2cam['C5C11']]['AImgsPath'],
                                                         fr+MCparams[cam2cam['C5C11']]['fr_offsetAVis']))
                c=0
                            ...
                            if metrics:
                                if bb[i, 0] == fr:
                                    ...
                                    if cam2cam == 'H5to11':
                                        #save log
                                        track_for_mot.writelines(
                                            str((bb[i, 0]+30)//15) + ',' + str(labelsC2[c]+1) + ',' + str(bb[i,1]) + ',' + str(
                                                bb[i,2]) + ',' + str(bb[i,3]) + ',' + str(bb[i,4]) + ',' +
                                                '-1' + ',' + '-1' + ',' + '-1' + ',' + '-1' + '\n')
                ...
                for bb in MCparams['H9to2']['PAproj'][len(labelsC2)::]:
                    # TODO: Set the colors of the rectangles
                    for i in range(1, len(bb)):
                        if fr_start <= bb[i, 0] <= fr:
                            # image = cv2.arrowedLine(image, start_point, end_point, color, thickness, tipLength = 0.5)
                            ...
                            # TODO: Think about how to show this only at the last detection in the current frame
                            if bb[i, 0] == fr :#and len(label_map_aux[c])>0
                                cv2.putText(imgC2, '{}'.format(labelsC9[c]), (int(bb[i - 1, 1]), int(bb[i - 1, 2])),
                                            cv2.FONT_HERSHEY_SIMPLEX, 3,  (255,0,255), 5, cv2.LINE_AA)
                    c = c + 1
                #show C5 projection (undistorted) on C2: visualize the projections on C2 from C5
                c = len(labelsC2)
                # tracks5 = tracks[C2+C9:C2+C9+C5]
                for bb in MCparams['H2to5']['PAproj'][len(labelsC2)::]:
                    # TODO: Set the colors of the rectangles
                    for i in range(1, len(bb)):
                        if fr_start <= bb[i, 0] <= fr:
                            # image = cv2.arrowedLine(image, start_point, end_point, color, thickness, tipLength = 0.5)
                            ...
                            # TODO: Think about how to show this only at the last detection in the current frame
                            if bb[i, 0] == fr :#and len(label_map_aux[c])>0
                                cv2.putText(imgC2, '{}'.format(labelsC5[c]), (int(bb[i - 1, 1]), int(bb[i - 1, 2])),
                                            cv2.FONT_HERSHEY_SIMPLEX, 3,  (255,0,255), 5, cv2.LINE_AA)
                    c = c + 1

                #show C11 projection (undistorted) on C5: visualize the projections on C5 from C11
                c = 0
                for bb in MCparams['H5to11']['PAproj'][:len(labelsC5)]:
                    # TODO: Set the colors of the rectangles
                    for i in range(1, len(bb)):
                        if fr_start <= bb[i, 0] <= fr:
                            # image = cv2.arrowedLine(image, start_point, end_point, color, thickness, tipLength = 0.5)
...
def plot_trajectories_pairs(labelCam2,
    ...
    assert len(MC_trklt_C9C2['PAproj'])==len(MC_trklt_C9C2['PAorg'])==MC_trklt_C9C2['trkltFamilySizeP']+MC_trklt_C9C2['trkltFamilySizeA']
    ...
    for fr in range(fr_start, fr_end):
        if (fr)%vis_rate==0:
            ...
            for bb in MC_trklt_C9C2['PAproj'][MC_trklt_C9C2['trkltFamilySizeP']:]: # show projection of auxiliary camera
                # TODO: Set the colors of the rectangles
                for i in range(1, len(bb)):
                    if fr_start <= bb[i, 0] <= fr:
                        if save_imgs and motion and bb[i, 0]%5==0:
...
def dafault_params(cam2cam, experiment, cams, mhtTrackers):
    ...
    if cam2cam == 'H9to2' and experiment == 'exp1':
        ...
        param['d_th'] = 0.17#0.17#0.17#0.45#200 # multi-task cost: 0.45, only loc: 0.17, loc+velocity 4d hd: 0.4
        param['primary'] = '/home/siddique/Desktop/NEU_Data/May_Demo_Trackers/exp2-train/cam02exp2.mp4testTracks.txt'
        ...
        param['PImgsPath'] =    benchmark + 'exp2'+ '/imgs/cam{:02d}'.format(cams['C2']) + 'exp2' + '.mp4/'#benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C2']) + 'exp1' + '.mp4/'
        #benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C2']) + tracklet_exp + '.mp4/'
        param['AImgsPath'] =   benchmark + 'exp2'+ '/imgs/cam{:02d}'.format(cams['C9']) + 'exp2' + '.mp4/' #benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C9']) + 'exp1' + '.mp4/'
    ...
    if cam2cam == 'H2to5' and experiment == 'exp1':
        ...
        param['PImgsPath'] =    benchmark + 'exp2'+ '/imgs/cam{:02d}'.format(cams['C2']) + 'exp2' + '.mp4/'#benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C2']) + 'exp1' + '.mp4/'
        #benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C2']) + tracklet_exp + '.mp4/'
        param['AImgsPath'] =   benchmark + 'exp2'+ '/imgs/cam{:02d}'.format(cams['C5']) + 'exp2' + '.mp4/' #benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C9']) + 'exp1' + '.mp4/'
        #benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C9']) + tracklet_exp + '.mp4/'
    ...
    if cam2cam == 'H5to11' and experiment == 'exp1':
        ...
        param['H'] = np.transpose(param['H'] )
        #param['H'] = np.linalg.inv(param['H'])
        param['d_th'] = 0.4 #0.17#0.17#0.45#200 # multi-task cost: 0.45, only loc: 0.17, loc+velocity 4d hd: 0.4
        ...
        #param['finalTrackFileC2'] = 'cam02exp2_MCTA.txt'
        param['PImgsPath'] =    benchmark + 'exp2'+ '/imgs/cam{:02d}'.format(cams['C11']) + 'exp2' + '.mp4/'#benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C2']) + 'exp1' + '.mp4/'
        #benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C2']) + tracklet_exp + '.mp4/'
        param['AImgsPath'] =   benchmark + 'exp2'+ '/imgs/cam{:02d}'.format(cams['C5']) + 'exp2' + '.mp4/' #benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C9']) + 'exp1' + '.mp4/'
        #benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C9']) + tracklet_exp + '.mp4/'
        param['outPath'] = benchmark + experiment + '/Results/MCTA/vis/cam{}to{}/'.format(cams['C5'], cams['C11'])

    if cam2cam == 'H11to13' and experiment == 'exp1':
        param = {}
        param['Pid'] = 11
        param['Aid'] = 13
        # C11: pri, C5: aux
       ...
       1.0e+03 *
        ...
        0.0011   -0.0001         0
        0.0001    0.0011         0
       -1.0947    0.0412    0.0010
        '''


        param['H'] = [[ 1.1,   -0.0001,   0.0],
                    [-0.0001,    1.1,    0.0 ],
                    [-1094.7,  41.2,    1.0 ]]
        ...
        param['PImgsPath'] =    benchmark + 'exp2'+ '/imgs/cam{:02d}'.format(cams['C11']) + 'exp2' + '.mp4/'#benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C2']) + 'exp1' + '.mp4/'
        #benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C2']) + tracklet_exp + '.mp4/'
        param['AImgsPath'] =   benchmark + 'exp2'+ '/imgs/cam{:02d}'.format(cams['C13']) + 'exp2' + '.mp4/' #benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C9']) + 'exp1' + '.mp4/'
        #benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C9']) + tracklet_exp + '.mp4/'
        param['outPath'] = benchmark + experiment + '/Results/MCTA/vis/cam{}to{}/'.format(cams['C13'], cams['C11'])

    if cam2cam == 'H13to14' and experiment == 'exp1':
        param = {}
        param['Pid'] = 14
        param['Aid'] = 13
        ...
        # param['finalTrackFileC2'] = 'cam02exp2_MCTA.txt'
        param['PImgsPath'] = benchmark + 'exp2' + '/imgs/cam{:02d}'.format(cams['C14']) + 'exp2' + '.mp4/'  # benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C2']) + 'exp1' + '.mp4/'
        # benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C2']) + tracklet_exp + '.mp4/'
        param['AImgsPath'] = benchmark + 'exp2' + '/imgs/cam{:02d}'.format(cams['C13']) + 'exp2' + '.mp4/'  # benchmark + 'test_data/exp1'+ '/imgs/cam{:02d}'.format(cams['C9']) + 'exp1' + '.mp4/'
        # benchmark + experiment + '/imgs/cam{:02d}'.format(cams['C9']) + tracklet_exp + '.mp4/'
...
def update_aux_ids(labelsC5,labelsC25,labelsC2, params):
    ...
    #-----------------------------------------------------------------------------------
    '''
    # tStart rules are not applied yet
    for k in labelsC5.keys():
        # TODO: consider different approach if minimum tStamp based id assignment does not works
        labelC5 = labelsC5[k][0]
        if len(labelC5) >= 2:  # association happened
            # select first appeared tracklets in solnPath to initialize id for remaining
            candIDkey = np.array(labelC5)[params['tStart'][k] == min(params['tStart'][k])][0]
            #check that candIDkey is within primary camera id keys
            if candIDkey in labelsC2.keys():
                labelsC5[k] = labelsC2[candIDkey]
            else:
                labelsC5[k] = candIDkey
        if len(labelC5) == 1:
            labelsC5[k] = labelsC5[k][0][0]
    '''
    #tStart rules are already applied
    # update labelsC5 based on labelsC2 in H9to2
    for k in labelsC5.keys():
        # TODO: consider different approach if minimum tStamp based id assignment does not works
        candIDkey = labelsC5[k]  # candKey is the values of aux but key for primary
        #check that candIDkey is within primary camera id keys
        if candIDkey in labelsC2.keys():
            labelsC5[k] = labelsC2[candIDkey]
        else:
            labelsC5[k] = candIDkey # keep aux label if not found in primary map
...
if __name__ == "__main__":
    ...
    label_map_update, MC_trklt_C9C2,fr_start, fr_end = get_cam2cam_matchedTrklt(MCparams[cam2cam['C9C2']], cam2cam, isDistorted=DistCorrect, isseqID=False)
    ...
    label_map_update, MC_trklt_C5C2, fr_start, fr_end = get_cam2cam_matchedTrklt(MCparams[cam2cam['C2C5']], cam2cam, isDistorted=DistCorrect, isseqID=False)
    labelsC5, labelsC25 = split_labelMap(label_map_update, MC_trklt_C5C2)
    labelsC5 = update_aux_ids(labelsC5,labelsC25,labelsC2, MC_trklt_C5C2)
    if cam2cam_vis:
    ...
    label_map_update, MC_trklt_C11C5, fr_start, fr_end = get_cam2cam_matchedTrklt(MCparams[cam2cam['C5C11']], cam2cam, isDistorted=DistCorrect, isseqID=False)
    #since primary change, make sure that prev and vur aux labels starting from same index
    labelsC511, labelsC11 = split_labelMap(label_map_update, MC_trklt_C11C5) # aux, pri = ()
    ...
    fr_end = 13400
    if cam2cam_vis:
        plot_trajectories_pairs(labelsC511,
                                labelsC11,
                                fr_start,
                                fr_end,
                                MC_trklt_C11C5,
    ...
    label_map_update, MC_trklt_C13C11, fr_start, fr_end = get_cam2cam_matchedTrklt(MCparams[cam2cam['C11C13']], cam2cam, isDistorted=DistCorrect, isseqID=False)
    #since primary change, make sure that prev and vur aux labels starting from same index
    labelsC1113, labelsC13 = split_labelMap(label_map_update, MC_trklt_C13C11) # aux, pri = ()
    #assert labelsC5.keys()[0]==labelsC511.keys()[0],'primary change, so current and previous auxiliary labelmap starting index should be equal but' \
                                                    #' found IndStartAprev {} and IndStartAcur {}'.format(labelsC5.keys()[0],labelsC511.keys()[0])
    #update primary cam ids
    #labelsC11 = update_pri_ids(pri_labels=labelsC11, aux_labels=labelsC511,aux_labels_prev=labelsC5)
    if cam2cam_vis:
        plot_trajectories_pairs(labelsC1113,
                                labelsC13,
                                fr_start,
                                fr_end,
                                MC_trklt_C13C11,
    ...
    label_map_update, MC_trklt_C13C14, fr_start, fr_end = get_cam2cam_matchedTrklt(MCparams[cam2cam['C13C14']], cam2cam, isDistorted=DistCorrect, isseqID=False)
    #since primary change, make sure that prev and vur aux labels starting from same index
